...
class Doctor
{
	...
	public function updateTempDocDetails($arr) {
        $update  =  $this->db->query("UPDATE user_temp_data set msg_status =  :msg_status where mobile = :mobile_num and id = :id ",$arr);
        if($update==1){
                return 1;
        }else{
                return 0;
	...
	public function getTempDocDetails($id=""){
		if($id){
			$array = array("mobile"=>$id);
			$result = $this->db->query("SELECT * FROM  user_temp_data WHERE mobile = :mobile",$array);
		}else{
			$result = $this->db->query("SELECT * FROM  user_temp_data");
		}
		return $result;
	}

	public function insertTempDocDetails($arr){
		...
		$arrDoc 	=  array(
			...
			"text_msg" => isset($arr['text_msg']) ? $arr['text_msg'] : "",
			"demo_date" =>$today,
			"demo_given_user" => isset($arr['userId']) ? $arr['userId'] : "",
			
		);
			
		$insert	 =  $this->db->query("INSERT INTO user_temp_data(first_name,last_name,mobile,text_msg,demo_date,demo_given_user) VALUES(:first_name,:last_name,:mobile,:text_msg,:demo_date,:demo_given_user)",$arrDoc); 
